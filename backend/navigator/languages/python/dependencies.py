...
# Common typing identifiers to ignore when extracting type dependencies
TYPING_IDENTIFIERS = {
    "List", "Dict", "Set", "Tuple", "Optional", "Union", "Annotated",
    "Literal", "Any", "Callable", "Iterable", "Iterator", "Mapping",
    ...
    def collect(self, tree):
        ...
        def walk(node):
            ...
            # from import statement: from x import y, z
                ...
                # Get the module name (first dotted_name or relative_import after 'from')
                for child in node.children:
                    if child.type == "dotted_name":
                        module_name = child.text.decode()
                        break  # CRITICAL: Stop after first dotted_name (the module)
                    elif child.type == "relative_import":
                        # Handle relative imports like 'from .module import X'
                        is_relative = True
                        # Find the dotted_name inside relative_import
                        for rel_child in child.children:
                            if rel_child.type == "dotted_name":
                                module_name = rel_child.text.decode()
                                break
                        break  # Stop after handling relative_import
                ...
                if module_name:
                    ...
                    if has_wildcard:
                        ...
                        # Get imported names
                        # For relative imports, all dotted_names at this level are imported names
                        # For absolute imports, skip the first dotted_name (module name)
                        seen_module = False
                        for child in node.children:
                            if child.type == "relative_import":
                                # Skip the relative_import node - we already processed it
                                continue
                            if child.type == "dotted_name":
                                if not is_relative and not seen_module:
                                    # First dotted_name in absolute import is the module, skip it
                                    seen_module = True
                                    continue
...
def resolve_dependencies(component, tree, source, all_components):
    ...
    # Fallback: if using Python AST (no tree-sitter), skip dependency resolution for now
    ...
    module_path = component.metadata.get("module_path", "")
    
    # Get all modules in the repository (both top-level and full paths)
    repo_modules = set()
    repo_module_prefixes = set()  # For nested module matching
    for cid in all_components:
        parts = cid.split(".")
        # Add the top-level module
        repo_modules.add(parts[0])
        # Add all possible module prefixes (e.g., for inventory.inventory_manager.Store, 
        # add "inventory" and "inventory.inventory_manager")
        for i in range(1, len(parts)):
            prefix = ".".join(parts[:i])
            repo_module_prefixes.add(prefix)

    def is_repo_module(module):
        """Check if a module path belongs to this repository"""
        # Direct match
        if module in repo_modules:
            return True
        # Check if it's a nested module path (like models.product)
        if module in repo_module_prefixes:
            return True
        # Check if it starts with a repo module
        top_level = module.split(".")[0]
        return top_level in repo_modules
    ...
    def resolve_name(name):
        ...
        if name in global_tracker.global_vars:
            # Align with DocAgent: do not track global var dependencies in tests
            if module_path.startswith("test"):
                return None
        ...
        for module, imported_names in import_tracker.from_imports.items():
            # Check if this is a repo module (handles nested modules like models.product)
            if is_repo_module(module):
    ...
    def collect_type_identifiers(type_node):
        ...
        def walk_type(n):
            if n.type == "identifier":
                ...
                # e.g., typing.List or models.Answer
                chain = extract_chain(n)
                if chain:
                    names.append(chain[-1])
            # Handle generics/subscripts like List[Answer]
            for c in n.children:
                walk_type(c)
    ...
    def walk(node):
        ...
        if node.type == "assignment":
            ...
            if lhs and rhs:
                ...
                # Pattern list (tuple unpacking): a, b = ... or (a, b) = ...
            ...
            # Handle type annotation in assignment (e.g., self._tx: Optional[Tx] = None)
            type_node = node.child_by_field_name("type")
            if type_node:
                for tname in collect_type_identifiers(type_node):
                    if tname in TYPING_IDENTIFIERS or is_ignored_name(tname):
                        continue
                    resolved = resolve_name(tname)
                    if resolved:
                        deps.add(resolved)

            return  # Don't walk children, we handled them manually

        # ---- Handle annotated assignments: x: Type or x: List[Type] ----
        if node.type == "type_definition":
            # Extract identifiers from the annotation
            annotation = node.child_by_field_name("type")
            for tname in collect_type_identifiers(annotation):
                if tname in TYPING_IDENTIFIERS or is_ignored_name(tname):
                    continue
                resolved = resolve_name(tname)
                if resolved:
        ...
        # ---- Handle for-loop variables: for x in ... or for a, b in ... ----
        if node.type == "for_statement":
            left = node.child_by_field_name("left")
            if left:
                if left.type == "identifier":
                    local_vars.add(left.text.decode())
                elif left.type in ("pattern_list", "tuple_pattern"):
                    for child in left.children:
                        if child.type == "identifier":
                            local_vars.add(child.text.decode())

        # ---- Handle except clause variables: except E as e ----
        if node.type == "except_clause":
            # Find the 'as' identifier (e.g., 'e' in 'except SysErr as e')
            found_as = False
            for child in node.children:
                if child.type == "as":
                    found_as = True
        ...
        if node.type == "attribute":
            ...
            if obj and obj.type == "identifier":
                ...
                if is_ignored_name(var):
                    ...
                    # Skip tracking test module globals to match DocAgent behavior
                    if not module_path.startswith("test"):
    ...
    # Pre-scan class body for annotated attributes (e.g., x: List[Answer])
    if component.type == ComponentType.CLASS:
        body = component_node.child_by_field_name("body")
        if body:
            for stmt in body.children:
                # Some grammars expose annotated assignment as 'type_definition';
                # to be robust, walk the full stmt to collect type identifiers
                if stmt.type == "type_definition" or stmt.type in ("expression_statement", "assignment"):
                    for tname in collect_type_identifiers(stmt):
                        if tname in TYPING_IDENTIFIERS or is_ignored_name(tname):
                            continue
                        resolved = resolve_name(tname)
                        if resolved:
    ...
    if component.type in (ComponentType.FUNCTION, ComponentType.METHOD, ComponentType.CONSTRUCTOR, ComponentType.API_ENDPOINT):
        ...
        if params:
            for child in params.children:
                if child.type == "identifier":
                    ...
                    # Add dependency from parameter type annotation
        ...
        # Function return type annotation (e.g., -> Answer)
        ret_node = component_node.child_by_field_name("return_type")
        for tname in collect_type_identifiers(ret_node):
            if tname in TYPING_IDENTIFIERS or is_ignored_name(tname):
                continue
            resolved = resolve_name(tname)
            if resolved:
    ...
    def walk(node, parent_class=None):
        ...
        if node.type == "class_definition":
            ...
            if name_node:
                ...
                if component.type == ComponentType.CLASS and component.id.endswith(f".{cname}"):
                    return node
                parent_class = cname
