"""
Comparative Analysis of Python Validation Results (The-Stack Dataset)

This script analyzes existing Python navigator results JSON and produces:
...
def analyze_results(results: Dict[str, Any]):
    ...
    print("\n" + "=" * 80)
    print("COMPARATIVE ANALYSIS: Python Navigator on The-Stack Dataset")
    print("=" * 80)
    ...
    if total > 0 and failed / total > 0.5:
        ...
        if error_summary.get('Not valid Python', 0) > failed * 0.7:
            print(f"     - Most failures: 'Not valid Python' parse errors")
            print(f"     - Issue: Dataset may contain non-Python code or invalid syntax")
            print(f"     - Solution: Improve non-Python detection or filter dataset")
        else:
    ...
    if successful == 0 and total > 0:
        ...
        print(f"     - Verify the-stack dataset includes Python samples")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print("  python analyze_python_validation_results.py data/validation/the-stack/python_navigator_results_100_samples.json")
        print("\nTo generate results first:")
        print("  python test_navigator_python_codesearchnet.py 100")
        print("\nAvailable results:")
        from pathlib import Path
        results_dir = Path("data/validation/the-stack")
