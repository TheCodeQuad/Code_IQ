"""
Comparative Analysis of TypeScript Validation Results (CodeSearchNet Dataset)

This script analyzes existing TypeScript navigator results JSON and produces:
...
def analyze_results(results: Dict[str, Any]):
    ...
    print("\n" + "=" * 80)
    print("COMPARATIVE ANALYSIS: TypeScript Navigator on CodeSearchNet Dataset")
    print("=" * 80)
    ...
    if total > 0 and failed / total > 0.5:
        ...
        if error_summary.get('Not valid TypeScript', 0) > failed * 0.7:
            print(f"     - Most failures: 'Not valid TypeScript' parse errors")
            print(f"     - Issue: Dataset may contain non-TypeScript code or invalid syntax")
            print(f"     - Solution: Improve non-TypeScript detection or filter dataset")
        else:
    ...
    if component_counts and max(component_counts) < 3:
        ...
        print(f"     - Adapter may be missing nested interfaces, types, or decorators")
        print(f"     - Check extractor for TypeScript-specific constructs")
    ...
    if successful == 0 and total > 0:
        ...
        print(f"     - TypeScript adapter may not be working properly")
        print(f"     - Check if TypeScriptAdapter is correctly parsing AST")
        print(f"     - Verify CodeSearchNet dataset includes TypeScript samples")
    
    if successful > failed and component_counts:
        avg = sum(component_counts) / len(component_counts)
        if avg > 3:
            print(f"  ✅ GOOD: High component extraction rate ({avg:.1f} avg)")
            print(f"     - Adapter is finding interfaces, classes, functions well")
            print(f"     - Consider testing on larger samples for robustness")
        if extraction_stats.get('interface', 0) > 0:
            print(f"  ✅ GOOD: TypeScript interfaces being extracted")
            print(f"     - Type definition extraction working properly")
    
    print(f"\n{'=' * 80}\n")

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python analyze_typescript_validation_results.py <results_file.json>")
        print("\nExample:")
        print("  python analyze_typescript_validation_results.py data/validation/codesearchnet/typescript_navigator_results_100_samples.json")
        print("\nTo generate results first:")
        print("  python test_navigator_typescript_codesearchnet.py 100")
        print("\nAvailable results:")
        from pathlib import Path
