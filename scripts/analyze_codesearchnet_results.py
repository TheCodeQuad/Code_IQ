"""
Comparative Analysis of CodeSearchNet JavaScript Validation Results

This script analyzes existing JS navigator results JSON and produces:
...
def analyze_results(results: Dict[str, Any]):
    ...
    print("\n" + "=" * 80)
    print("COMPARATIVE ANALYSIS: JavaScript Navigator on CodeSearchNet")
    print("=" * 80)
    ...
    for status, results_list in sorted(by_status.items()):
        ...
        if status == 'success' and results_list:
            print(f"    → Sample indexes: {[r.get('sample_index') for r in results_list[:5]]}")
    ...
    if total > 0 and failed / total > 0.5:
        ...
        if error_summary.get('Not valid JavaScript', 0) > failed * 0.7:
            print(f"     - Most failures: 'Not valid JavaScript' parse errors")
            print(f"     - Issue: Dataset may contain non-JS code or invalid syntax")
            print(f"     - Solution: Improve non-JS detection or filter dataset")
        else:
    ...
    if component_counts and max(component_counts) < 3:
        ...
        print(f"     - Adapter may be missing nested functions")
        print(f"     - Check extractor for anonymous/arrow functions")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python analyze_codesearchnet_results.py <results_file.json>")
        print("\nExample:")
        print("  python analyze_codesearchnet_results.py data/validation/codesearchnet/js_navigator_results_100_samples.json")
        print("\nTo generate results first:")
        print("  python test_navigator_js_codesearchnet.py 100")
        print("\nAvailable results:")
        from pathlib import Path
